...
const KeyTabContent: React.FC<KeyTabContentProps> = ({
  state,
  setState,
  onPreview,
}) => {
  ...
  return (
    <div className="flex flex-col gap-[19px]">
      ...
      <div className="flex justify-between w-full items-center">
        <p className="text-white text-style-2">{t("keySetting.keyMapping")}</p>
        <button
      ...
      {/* 커스텀 이미지 */}
      <div className="flex justify-between w-full items-center">
        <p className="text-white text-style-2">{t("keySetting.customImage")}</p>
        <button
          ref={imageButtonRef}
          type="button"
      ...
      {useCustomCSS && (
        <div className="flex justify-between w-full items-center">
          <p className="text-white text-style-2">{t("keySetting.className")}</p>
          <input
            type="text"
      ...
      {state.showImagePicker && (
        <ImagePicker
          ...
          idleImage={state.inactiveImage}
          activeImage={state.activeImage}
          idleTransparent={state.idleTransparent}
          activeTransparent={state.activeTransparent}
          ...
          onClose={() =>
            setState((prev) => ({ ...prev, showImagePicker: false }))
          }
        />
      )}
    </div>
