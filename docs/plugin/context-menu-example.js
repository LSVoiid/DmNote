...
(function () {
  ...
  menuIds.push(
    window.api.ui.contextMenu.addKeyMenuItem({
      ...
      onClick: async (context) => {
        ...
        if (confirmed) {
          console.log(
            `[Context Menu Example] 카운트 초기화: ${context.keyCode}`
          );
  ...
  // ============================================================
  highlightMenuId = window.api.ui.contextMenu.addKeyMenuItem({
    id: "toggle-highlight",
    label: "하이라이트 모드 켜기",
    position: "bottom",
    onClick: async () => {
      ...
      // 스토리지에 상태 저장
      await window.api.plugin.storage.set("highlightMode", highlightMode);

      // 메뉴 라벨 업데이트
      window.api.ui.contextMenu.updateMenuItem(highlightMenuId, {
        label: highlightMode ? "하이라이트 모드 끄기" : "하이라이트 모드 켜기",
      });

      console.log(
        `[Context Menu Example] 하이라이트 모드: ${
  ...
  menuIds.push(
    window.api.ui.contextMenu.addGridMenuItem({
      ...
      onClick: async (context) => {
        ...
        // 스토리지에 마커 저장
        const markers =
          (await window.api.plugin.storage.get("noteMarkers")) || [];
        markers.push({
          x: dx,
          y: dy,
          mode: context.mode,
        ...
        await window.api.plugin.storage.set("noteMarkers", markers);
  ...
  (async () => {
    try {
      ...
      const savedHighlightMode =
        (await window.api.plugin.storage.get("highlightMode")) || false;
