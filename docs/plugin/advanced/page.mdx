...
<Callout type="warning">
  직접 DOM 조작 시 반드시 `registerCleanup`으로 요소를 정리해야 합니다.
</Callout>
...
```javascript
// 플러그인 정보
console.log("플러그인 ID:", dmn.plugin.id);
...
defineElement({
  ...
  template: ({ count }) => html`<span>${count}</span>`,
});
```
...
// 좋은 예: 한 번에 업데이트
Object.assign(ctx.state, { a: 1, b: 2, c: 3 });
```
